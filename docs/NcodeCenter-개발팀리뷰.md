...
|------------------|----------------------|------|------|
| **Ticket 발급** | 티켓 발급 (`TKT-01`) | 🟰 동일 계승 | N Key · **계정+App Key**(`TKT-01-2`) · 발급목록에 **정산/금액** 추가 |
...
| **Ncode Info** | Ncode 정보 (`INF-01`) | 🟰 동일 | 섹션표 · 발급구조 · 가이드(탭 통합) |
...
> **User 관리 해석**: Admin의 `User 관리`는 **계정이 필요한 대상(user)** 을 뜻한다. 실은 **Company(고객사)에 등록하면 될 것을 굳이 별도 메뉴로 분리**했던 것이다. NcodeCenter는 이를 **고객사 관리(CLI-01) 한 곳으로 통합(정상화)** 했다 — 계정(App Key) 부여는 티켓 발급의 `계정 + App Key`(TKT-01-2)에서 처리.
...
flowchart LR
  ...
  B --> D["③ 티켓 발급<br/>TKT-01 · N Key/App Key/정산"]
