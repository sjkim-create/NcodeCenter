...
SOUND_Q = (('Ncode 적용', 500, 240), ('Ncode 편집(기본)', 1000, 1240), ('Compound 2언어', 1300, 0),
           ...
           ('프롬프트 편집', 50000, 0), ('발음평가 편집', 50000, 0))
    ...
    pen_models = ('<div style="display:grid;'
                  ...
                  % (field('발급인 (코드 할당자 · 사용자 명단)', sel('김순정'), False, None,
                     ...
                     field('펜 모델 · 다중 선택',
                           ...
                           '목록 맨 아래 <b>[＋ 직접 추가…]</b> 로 <b>새 모델을 직접 적을 수 있다</b> '
                           '<code>PC-100</code>')))
...
def totals(pen='소리펜'):
    ...
    return ('<div class="card" style="background:#f5f9ff;border-color:#bfdbfe">'
            ...
            '<span style="color:#9ca3af;font-size:11px"> p · 적용비 기준</span></span>'
...
def build():
    ...
    boards.append((
        ...
        [('S / O', '—', '<b>수정 불가</b>', '수정 모드에서는 변경할 수 없다'),
         ('Book', '선택', '—', '사용 가능한 번호에서'),
         ('발급인 · 펜 모델', '—', '<b>한 행</b>',
          '교재명 다음 줄에 <b>발급인(앞) · 펜 모델(뒤)</b> 순으로 나란히 둔다 <code>PC-096</code>'),
         ('펜 모델 [＋ 직접 추가…]', '선택', '입력칸으로 바뀜',
          '목록에 없는 모델을 <b>직접 적어 추가</b>한다 <code>PC-100</code> — '
          '[추가]/Enter 로 확정, [취소]/Esc 로 되돌린다. '
          '추가한 모델은 <b>다음에도 셀렉트에 나온다</b>(브라우저 보관 · 교재 공용)'),
         ('펜 모델', '셀렉트에서 선택', '칩으로 추가',
          '<b>발급인 오른쪽</b>에 있다 <code>PC-095</code> <code>PC-096</code> · <b>[＋ 펜 모델 추가…]</b> 에서 고르면 '
