# -*- coding: utf-8 -*-
"""MEM-02 고객사 등록·수정 (MEM-03 업무요청 메모 포함)"""
...
def price_card(n_custom=3):
    ...
    def group(title, items):
        ...
        return ('<div style="margin-top:8px">'
                ...
                '%s</div>'
                '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px 12px">%s</div>'
                '</div>' % (title, cells))
    ...
    return ('<div class="card"><div class="hd">편집 단가 (2026 항목별) '
            ...
            '</div></div>'
            % (badge, group('🔊 소리펜', PRICE_NSP), group('✍ 필기펜', PRICE_NWP)))
...
def memo_card(draft='', hi=None):
    ...
    return ('<div class="card" style="height:100%%"><div class="hd">업무 원장 '
            '<span class="tag">MEM-03</span><div class="sp"></div>'
            '<span style="font-size:11px;color:#9ca3af;font-weight:400">수정 모드 전용</span></div>'
...
def footer(save_label='저장'):
    return ('<div class="row" style="justify-content:flex-end;gap:8px;margin-top:14px">'
            '<div class="btn gho">취소</div><div class="btn pri">%s</div></div>' % save_label)


def reg_mode(name_err=False):
    return (basic_card(name_err=name_err, empty=not name_err) + '<div style="height:14px"></div>'
            + common_card() + '<div style="height:14px"></div>'
            + price_card(0) + '<div style="height:14px"></div>' + docs_card() + footer('등록'))


def edit_mode(is_parent=False, closed=False, n_custom=3, draft='', hi=None):
    left = (basic_card(closed=closed) + '<div style="height:14px"></div>'
            + common_card(is_parent) + '<div style="height:14px"></div>'
            + price_card(n_custom) + '<div style="height:14px"></div>' + docs_card() + footer())
    return ('<div style="display:grid;grid-template-columns:1fr 560px;gap:14px;align-items:start">'
            '<div>%s</div><div>%s</div></div>' % (left, memo_card(draft, hi)))
...
def build():
    ...
    boards.append((
        ...
        [('업체명', '입력 (필수)', '—', '코드·프로젝트가 이 이름으로 연결된다'),
         ...
         ('주소', '입력', '—', '<code>TKT-03</code> 계정 등록 시 <b>기본 주소</b>로 사용된다'),
         ('은행명 / 계좌번호', '입력', '—', '정산 지급용'),
    ...
    boards.append((
        ...
        frame('MEM-01', '고객사 수정', edit_mode(is_parent=True), height=1580),
    ...
    boards.append((
        ...
        frame('MEM-01', '고객사 수정', edit_mode(n_custom=3), height=1580),
    ...
    boards.append((
        ...
        '코드 발급 이력은 <b>보존</b>된다.',
        frame('MEM-01', '고객사 수정', edit_mode(closed=True), height=1640),
    ...
    boards.append((
        ...
        frame('MEM-01', '고객사 수정', edit_mode(hi='1'), overlay=err_ovl, height=1580),
    ...
    boards.append((
        ...
        frame('MEM-01', '고객사 수정', edit_mode(), overlay=memo_del, height=1580),
    ...
    intro = ('고객사의 <b>기본 정보 · 공통코드 귀속 · 편집 단가 · 관련 서류</b>를 입력한다. '
             ...
             '<code>MEM-01</code>의 [＋ 고객사 등록] = 등록(1단), 행 클릭 = 수정(2단 · 업무 원장 <code>MEM-03</code> 포함). '
             '단가는 <b>3단 구조</b>(기본 단가 → 고객사 단가 → 교재별 할인)이며, '
             '변경은 <b>이후 등록되는 교재부터</b> 적용되고 기존 교재는 등록 당시 단가를 유지한다 <code>PC-027</code>.')
