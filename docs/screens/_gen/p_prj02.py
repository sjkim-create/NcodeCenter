        ...
        for (no, st, cu, t, fn, k, ty, (s, o, b), prange, pg, sym, mth, d, nmod,
            ...
            if share:
                if cu:
                    ...
                    v = ('<span style="color:#d97706" title="사용 고객사가 '
...
def build2():
    ...
    B.append((
        ...
        [('신규 배지', '표시', '—', '이 화면에서 추가한 항목만'),
         ('[삭제]', '클릭', 'S11 확인창',
          '그 고객사·<b>추가한 교재</b>·<b>SOBP 맵 편집 표시</b>가 함께 사라진다 <code>PC-075</code>'),
    ...
    B.append((
        'S11', '고객사 삭제 확인창', '확인창',
        '사용자가 추가한 고객사를 지운다. 삭제는 <b>[＋ 고객사] 로 등록한 것을 그대로 되돌린다</b> '
        '<code>PC-075</code> — 목록·<b>추가한 교재</b>·<b>SOBP 맵 Book 오버라이드</b>·'
        '<b>코드 프로젝트 편집 플래그</b>가 함께 사라지므로 확인창으로 한 번 막는다.',
        frame('PRJ-02', '편집 프로젝트', content(sel_cust='시원스쿨'),
              overlay=confirmbox('이 고객사를 편집 프로젝트에서 삭제할까요?'), height=H2),
        [('[삭제]', '클릭', '확인창',
          '<b>이 고객사를 편집 프로젝트에서 삭제할까요?</b> / '
          '<b>추가한 교재와 SOBP 맵의 편집 표시도 함께 지워집니다.</b>'),
         ('[삭제] 확정', '클릭', '목록에서 제거',
          '선택 중이었으면 <b>장부의 첫 고객사</b>로 옮겨간다'),
         ('되돌리는 것', '—', '<b>4가지</b>',
          '① 고객사 카드 ② 추가한 교재 ③ SOBP 맵 Book 오버라이드(사용 고객사·편집 표시) '
          '④ 코드 프로젝트 편집 플래그 → <b>OWNER 카드의 [편집] 배지가 사라진다</b> <code>PC-075</code>'),
         ('남는 것', '—', '<b>장부 편집 이력</b>',
          '편집현황 장부에 편집 이력이 있는 좌표는 계속 <b>[편집]</b> — 실제 편집량이라 잔재가 아니다'),
         ('[취소]', '클릭', '<code>PRJ-02</code>', '변경 없음'),
...
def build3():
    ...
    B.append((
        'S5', '사용 고객사 열 — 공유 코드 · 사용 고객사가 있는 고객사', '분기',
        '<code>P-12</code> — 공유(커먼) 코드를 쓰는 Owner 거나, <b>교재에 사용 고객사 값이 하나라도 있는 고객사</b>'
        '(예: 네오노트-14-27)면 <b>교재명 왼쪽에 「사용 고객사」 열</b>이 추가되고 그 열의 필터도 함께 나타난다 '
        '<code>PC-109</code>. 검색창도 <b>교재명·사용고객사 검색</b> 으로 바뀐다. '
        '값이 비면 공유 코드든 아니든 <b>미입력</b>(주황)으로 표시된다. 필터에는 <b>(미입력)</b> 이 따로 있어 '
        '비어 있는 교재만 골라 볼 수 있다.',
        frame('PRJ-02', '편집 프로젝트',
              content(share=True, sel_cust='NeoLAB'), height=H3),
        [('사용 고객사 열', '표시', '—', '공유 코드 또는 사용 고객사 값이 있는 고객사 · 보라 배지'),
         ('미입력', 'hover', '툴팁',
          '<b>사용 고객사가 비어 있습니다. 교재를 열어 입력하세요.</b>'),
         ('사용 고객사 필터', '선택', '행 필터', '그 열 아래 드롭다운 — <b>전체 · (미입력) · 고객사명</b> <code>PC-109</code>'),
         ('(미입력)', '선택', '빈 교재만', '사용 고객사가 비어 있는 교재만 남는다'),
