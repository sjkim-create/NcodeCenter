...
- 경로 B로 나간 코드도 결국 "어느 고객사(Owner)의 어떤 서비스"인지 남아야 하므로, NcodeCenter에서는 사용 서비스 **`NCODEPRINTER` (Ncode 프린터(드라이버))** 로 구분한다. (PC-019)
...
| 항목 | 반영 위치 | 상태 |
|---|---|---|
| 사용 서비스 `NCODEPRINTER` | `web/lib/customerData.ts` `SERVICE` · `[SOBP 맵] ▸ 직접 코드 할당` · `[코드 프로젝트]` 서비스 필터 · `lib/dbSchema.ts` | ✅ 반영(PC-019) |
